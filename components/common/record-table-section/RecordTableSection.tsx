...
interface RecordTableSectionProps {
  ...
  rows: RecordTableRow[];
  onAdd?: () => void;
  // Label for the add button. Defaults to "Add" so existing call
  // sites (e.g. Guardians) are unaffected; override for tables where
  // a more specific label reads better (e.g. "Add Optional Subject").
  addLabel?: string;
  onEditRow?: (rowId: string) => void;
  onDeleteRow?: (rowId: string) => void;
  deletingRowId?: string | null;
  emptyLabel?: string;
  // When true (default), delete is hidden on the last remaining row —
  // matches the original behavior for tables like Registration
  // Number/Location where at least one record must always exist. Set
  // to false for tables where zero rows is a valid state (e.g.
  // Guardians — a student can have none).
    ...
    <section className="flex w-full flex-col gap-4">
      ...
      <div className="w-full rounded-2xl border border-primary-100 bg-primary-bg p-2">
        ...
        {rows.map((row, index) => {
          ...
          const canDelete = requireAtLeastOne ? rows.length > 1 : true;
